...
export const generateMetadata = async ({
  ...
  return {
    title: generateMetadataTitle(customer?.name ? customer.name : "Cliente"),
    description: `Ver y administrar el perfil, las citas y los registros de ${customer?.name ? customer.name : "el cliente"}. Esta página ofrece una visión general completa de la información e historial del cliente.`,
  };
};
...
export default async function CustomerPage({
  ...
  const { data: customerAppointments, totalPages } =
    await customersService.getCustomerAppointments(
      customerId,
  ...
  return (
    <section className="h-full w-full flex flex-col items-center py-8 md:p-12 space-y-8">
      ...
      <EditAppointmentDialog services={services} />
      <EditCustomerDialog />
    </section>
  );
}
