...
export default async function AdminPage() {
  ...
  const expectedData = await financeService.getExpectedData(
    startOfDay(today),
    endOfDay(today),
  );
  ...
  return (
    <section className="h-full w-full flex flex-col items-center justify-center py-8 md:p-12">
      <Container className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <DailyAppointmentsSection appointments={todayAppointments} />
        <QuickActions today={today} services={services} />
        <DailyIncome
          day={today}
