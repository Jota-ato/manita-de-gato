        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          {activeServices.length ? (
            activeServices.map((service) => (
              <ServiceCard key={service.data.id} service={service} isAdmin />
            ))
          ) : (
            <Heading level={2} className="sm:col-span-2 md:col-span-3 my-4">
              No hay servicios activos
            </Heading>
          )}
          <AddServiceCard extras={extras} />
        </div>
      </TabsContent>
      ...
      <TabsContent className="min-h-120" value="activeExtras">
        <div className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 gap-4">
          {activeExtras.length ? (
            ...
            <Heading level={2} className="sm:col-span-3 md:col-span-4 my-4">
              No hay extras
            </Heading>
          )}
          <AddExtraCard />
