...
export function Agenda({
  ...
  const [startHourHours, startHourMinutes] = safeStartHour
    .toISOString()
    .split("T")[1]
    .split(":")
    .map(Number);
  ...
  const visibleEvents = useMemo(
    () =>
      events.filter((event) => {
        ...
        const isValid = eventStart <= endOfView && eventEnd >= startOfView;
        return isValid;
      }),
  ...
  const hours = Array.from({ length: slotsPerDay }).map((_, i) =>
    addHours(
      startOfDay(startOfView),
      startHourHours + startHourMinutes / 60 + (slotDuration / 60) * i,
    ),
  );
