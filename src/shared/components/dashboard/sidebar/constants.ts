...
export const navigation: navigationType[] = [
  ...
  {
    label: "Controles de negocio",
    href: "/dashboard/business-controls",
    icon: LuBriefcaseBusiness,
  },
  { label: "Moldes", href: "/dashboard/molds", icon: LuCalendarSearch },
];
...
export const generateNavigation = (role: UserRole): navigationType[] => {
  ...
  if (role === "employee") {
    return navigation.filter(
      (item) =>
        !["Finanzas", "Registro", "Usuarios", "Clientes", "Moldes"].includes(
          item.label,
        ),
    );
  }
